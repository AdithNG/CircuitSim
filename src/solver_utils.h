...
inline std::vector<std::complex<double>> solve_complex_linear_system(
    ...
    for (std::size_t pivot = 0; pivot < size; ++pivot) {
        ...
        for (std::size_t candidate = pivot + 1; candidate < size; ++candidate) {
            const double candidate_value = std::abs(matrix[candidate][pivot]);
            if (candidate_value > best_value) {
                best_value = candidate_value;
                best_row = candidate;
            }
        ...
        const std::complex<double> pivot_value = matrix[pivot][pivot];
        for (std::size_t row = pivot + 1; row < size; ++row) {
            const std::complex<double> factor = matrix[row][pivot] / pivot_value;
    ...
    std::vector<std::complex<double>> solution(size, std::complex<double>{0.0, 0.0});
    for (std::size_t row = size; row-- > 0;) {
